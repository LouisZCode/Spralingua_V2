...
- pattern verben-mit-praepositionen · reference "bei dem" · typed "dir beim" (frame "Ich helfe ___ Umzug.") → **correct: true**. The learner put an ADDITIONAL correct word in the gap — the dative object "helfen" takes anyway. "Ich helfe dir beim Umzug" is textbook German and "helfen bei" + Dativ is fully exercised. An extra word is free when it is correct German and the chunk is still there. (PROD 2026-08-14: a learner was failed on exactly this.)
- pattern verben-mit-praepositionen · reference "bei dem" · typed "mich beim" → **correct: false**. CONTROL: the extra word is WRONG German — helfen takes the dative ("mir"/"dir"), "mich" is accusative — so prong 1 fails.
- pattern verben-mit-praepositionen · reference "an die" · typed "an mein" (frame "Ich denke oft ___ Zeit in Spanien.") → **correct: false**. CONTROL: possessive is free, but "Zeit" is feminine and "mein" lacks the -e ("meine") — a wrong ENDING inside the gap always matters.
