...
- Deploys drain instead of cutting live voice sessions (a SIGTERM lets each session say goodbye, bounded by `SHUTDOWN_DRAIN_S`), and `scripts/pg_dump_to_bucket.py` is the offsite Postgres backup meant to run as a weekly Railway cron on the backend image.
