...
def _matches(typed: str, expected: str) -> bool:
    """Deterministic green: the exact phrase, or the phrase embedded on word
    boundaries in a longer answer (learners often type the whole sentence)."""
    t, e = _normalize(typed), _normalize(expected)
    if t == e:
        return True
    return re.search(rf"(?<!\w){re.escape(e)}(?!\w)", t) is not None
...
async def submit_attempt(
    ...
    with tracer.start_as_current_span("bauteil-attempt") as attempt_span:
        ...
        if _matches(answer, item["answer"]):
